...
public class Problem {
	...
	public static List<String> getZipsOfSpecialCustomers(List<Customer> list) {
		//implement
		Predicate<Customer> containsMoreThan6Char = customer -> customer.getCity().length()>6;
		Predicate<Customer> ContainE = customer -> customer.getCity().contains("e");
		return list.stream()
				.filter(containsMoreThan6Char.and(ContainE.negate()))
				.map(Customer::getZip)
				.distinct()
				.collect(Collectors.toList());
	}
